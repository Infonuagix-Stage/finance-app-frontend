...
// Define the types for record and props
...
const CategoryPage = () => {
  ...
  // Fix: Ensure userId is never null, only undefined or string
  const userId: string | undefined = user?.sub ?? undefined;

  const { categoryId, year: initialYear, month: initialMonth } = state || {};
  const categoryType: "INCOME" | "EXPENSE" | undefined = state?.categoryType as "INCOME" | "EXPENSE" | undefined;
          ...
          <ul className="space-y-4">
            {filteredRecords.map((rec) => (
              ...
              >
                ...
                <RecordActions
                  onEdit={() => setEditingRecord(rec as TransactionRecord)}
                  onDelete={() =>
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-gray-800 p-6 rounded-lg shadow-lg w-96">
            ...
            <div className="flex justify-end space-x-4">
              ...
              <button
                onClick={() => {
                  handleEditRecord(editingRecord.expenseId ?? editingRecord.incomeId ?? "", {
                    ...editingRecord,
                    categoryId,
                    userId,
                    expenseDate: editingRecord.expenseDate ?? "",
                  });
