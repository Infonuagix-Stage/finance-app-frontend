...
export const getExpensesFiltered = async (userId, categoryId, month, year) => {
 
  const response = await api.get(
    `/users/${userId}/categories/${categoryId}/expenses/filter`,
    {
      params: { month, year },
    }
  );
  return response.data; // Devrait être une liste de ExpenseResponseDTO
}
